...
parser.add_argument(
    "-i",
    "--input-directory",
    help="Chemin du dossier contenant les fichiers sources Markdowns",
)
parser.add_argument(
    "-o",
    "--output-directory",
    help="Chemin du dossier qui contiendra les fichiers générés",
)
parser.add_argument(
    "-t", "--template-directory", action="store_true", help="Chemin du dossier template"
)
parser.parse_args()
    ...
    with open(fichier_source, "r", encoding="utf-8") as mon_fichier:
        for ligne in mon_fichier:
            ...
            while "*" in new_ligne:
                ...
                new_ligne = (
                    new_ligne[:index_etoile] + "<em>" + new_ligne[index_etoile + 1 :]
                )
                index_etoile = new_ligne.index("*")
                new_ligne = (
                    new_ligne[:index_etoile] + "</em>" + new_ligne[index_etoile + 1 :]
        ...
        nom_fichier_destinataire = str(fichier_source)[
            str(fichier_source).rindex("/") + 1 : str(fichier_source).index(".")
        ]
        with open(
            "{}.html".format(nom_fichier_destinataire), "w", encoding="utf-8"
